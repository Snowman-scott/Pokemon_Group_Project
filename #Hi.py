...
def get_pokemon(name):  #2nd finds the pokemon inside the pokedex dictionary
...
def GetUserInput(): 
    while True:
        playerChoice = input("Pick a Pokémon: ").capitalize() #function 3rd where the user picks their pokemon
...
def battle(player_pokemon_name, enemy_pokemon_name): #4th gets the data from both pokemons, if its valid or not
    ...
    while player_hp > 0 and enemy_hp > 0:  #7th the battle loop continues until the health is not above 0
        print(f"Your HP: {player_hp} | Enemy HP: {enemy_hp}") #8th the players turn
        action = input("(A)ttack or (H)eal? ").lower()

        if action == "a":
            base_damage = random.randint(8, 15) #9th if the player chooses to attack
...
print("You have entered a battle")  #start 1st gettig user input for pokemon
player_choice = GetUserInput()

#Random enemy Pokémon
